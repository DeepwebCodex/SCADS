...
<ul class="page-nav fl">
			...
			<li class="active">Domains where interesting links were found potentially vulnerable</li>
...
//echo '<br>';
	...
	echo "<td>{$domen_new} <span style='color:blue;font-weight:700'>({$cc})</span><a href=\"#\" onclick=\"ShowDiv({$id}); return false\">Open/</a><a href=\"#\" onclick=\"ShowDiv2({$id}); return false\">Close</a>/ <a target='_blank' href=\"{$url}\">URL</a></td>";
	...
	echo "<td>{$date}</td>";
	 ...
	 echo '<td width="40">'.$ajax->link("DEL", '/posts/shlak_domen/'.$id,array('class'=>'icon','title'=>'Move to bad','escape' => false));
